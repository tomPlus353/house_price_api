...
Sends a POST to `$APP_URL` with the provided
...
BASE_URL = "https://tomplus353--ml-model-api-predict.modal.run"
...
def parse_prediction(pred_str: str) -> float:
    ...
    if isinstance(pred_str, (int, float)):
...
def run_single_test(
    case: Dict[str, Any], base_url: str = BASE_URL
) -> Dict[str, Any]:
